//열거형 상수 : 성적관리 주메뉴 선택 사용
enum MAIN_MENU {END, READ, PRINT, EDIT, WRITE}; //가독성을 높이기 위해서 실무에서 많이쓰인다.
enum MAIN_MENU main_select;

//열거형 상수 : 성적관리 부메뉴 선택 사용
enum EDIT_MENU {EXIT, PREV, NEXT, INSERT, MODIFY, DELETE, SEARCH};
enum EDIT_MENU edit_select;

#include "student_dll.h"
void read_student_data(); // student_data.txt 파일에서 데이터(레코드) 읽어오기
void write_student_data(); // student_data.txt 파일에서 데이터(레코드) 저장하기
char student_file_name[20] = "student_data.txt";
...
void read_student_data(){
    ...
    printf("읽어올 학생 정보 파일이름을 입력>>");
    scanf_s("%s",student_file_name, sizeof(student_file_name)); //scanf_s("%s",&student_file_name[0], sizeof(student_file_name));
    getchar();
    ...
    while(!flag){
        ...
        int result = fscanf(file,"%s %d %s %d %d %d %d %d %d %lf %c",
                ...
                &data->kotlin, &data->react_native, &data->javascript,
                &data->total, &data->avr, &data->grade);

        if(result == EOF){
            flag = 1;
            continue;
        }
            append_node(data); //이중연결리스트 데이터 구조체 추가 연결.
            count++;
...
void edit_student_data(){
    STUDENT_t* data = NULL;
    current = head->next; //실제 데이터가 저장된 노드위치 저장 // current는 전역변수로 저 위에서 썼음
    int flag = 0;
    ...
    while(!flag && head != NULL){
        ...
        switch(edit_select){
            ...
            case INSERT:
                ...
                //1,2,3,4 노드삽입 작전 1. head 삽입, 2:마지막에 노드 삽입, 3:처음 노드 삽입, 4:중간 노드 삽입
