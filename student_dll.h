...
typedef struct STUDENT{
    ...
    struct STUDENT* prev; // struct STUDENT를 가르키는 포인터변수(바로앞에 있는 구조체)
    struct STUDENT* next; // struct STUDENT를 가르키는 포인터변수(뒤에 있는 구조체)


}STUDENT_t;


STUDENT_t* head = NULL; //이중연결리스트노드
STUDENT_t* current = NULL; //현재 화면에 표시되는 노드를 저장한다.
...
void append_node(STUDENT_t* data){
   ...
   //next가 NULL인 마지막 노드로 위치이동
    while(p->next != NULL){
        p = p->next;
    }
    p->next = data;
    // 추가된 node에서 바로 앞 node를 가르킴.
    if(p != head){
        data->prev = p;
    }
return;
...
void print_student_data(){
    ...
    while(p->next != NULL){
            p = p->next;
            printf("%s\t%d\t%s\t%3d %3d %3d %3d %3d\t%d\t%5.2lf\t%2c\n",
                p->name, p->id, p->gender, p->java, p->android,
                p->kotlin, p->react_native, p->javascript,
                p->total, p->avr, p->grade);
    }
return;
}
...
void update_student_data(STUDENT_t* current){
        if(current == NULL){
            puts("수정할 STUDENT 구조체가 존재하지 않습니다.");
            getchar();
            return;
        }
        ...
        current->react_native = input_score("수정 리엑트네이티브");
...
void  insert_student_data(STUDENT_t* data){
    // 노드삽입 작전 1번:head ->next null  처음 위치 삽입
    if(head->next == NULL){
        head->next = data;
        return;
    }
    // 2:마지막에 노드 삽입, 3:처음 노드 삽입, 4:중간 노드 삽입
    if(current->prev == NULL){    // 3: 처음노드삽입
        data->next = head->next;
        data->next->prev = data;
        head->next = data;
        ...
        data->prev = current;
        current->next = data;
    }else if(current->next !=NULL && current->prev != NULL){ // 4:중간 노드삽입
...
void delete_student_data(){
    ...
    //current 노드가 삭제되기 때문 current 이전노드, 다음노드, NULL 변경
    ...
    if(p->next != NULL && p->prev == NULL){        // 처음노드
        ...
        p->next->prev = p->prev;
        p->prev->next = p->next;
    }else if(p->next == NULL && p->prev != NULL){ // 마지막노드
        p->prev->next = NULL;
    }else{
        head->next = NULL;
    }
